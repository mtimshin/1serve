  ...
  <div v-if="page" class="article">
    <div class="article__top">
      <div class="container">
        <NuxtLink to="/tournaments" class="article__back">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></polyline><polyline points="12 19 5 12 12 5"></polyline></svg>
          Вернуться назад
        </NuxtLink>
    ...
    <div class="container article__layout">
      <aside class="sidebar">
        <div class="sidebar__mobile-header">
          <button 
            ...
            :class="{ 'sidebar__header--active': isTocOpen }"
            @click="toggleToc"
          >
            Содержание
          </button>
        ...
        <ul class="toc" :class="{ 'toc--active': isTocOpen }">
          <li v-for="link in tocLinks" :key="link.id" class="toc__item">
            <a 
              :href="`#${link.id}`" 
              class="toc__link" 
              :class="{ 'toc__link--active': activeSection === link.id }"
      ...
      <div class="content article__body">
        ...
        <div v-if="page.gallery" class="gallery-section">
          <h2>Галерея событий</h2>
          <ImageSlider :images="page.gallery" />
        </div>
      </div>
    </div>
